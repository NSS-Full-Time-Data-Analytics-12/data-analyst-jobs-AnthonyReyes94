...
SELECT COUNT(company) AS num_of_post_TN
FROM data_analyst_jobs
WHERE location = 'TN'

--21

SELECT COUNT(company) AS num_of_post_TN_or_KY
FROM data_analyst_jobs
...
SELECT COUNT(star_rating) AS num_posts_TN_above_4_star
FROM data_analyst_jobs
WHERE location = 'TN'
	AND star_rating > 4;
...
SELECT COUNT(review_count) AS review_count_500_to_1000
FROM data_analyst_jobs
...
SELECT COUNT(DISTINCT(title)) AS job_titles_CA
FROM data_analyst_jobs
WHERE location = 'CA';
...
SELECT COUNT(DISTINCT title) AS not_analyst_or_analytics
FROM data_analyst_jobs
...
SELECT domain, COUNT(skill) AS require_SQL
FROM data_analyst_jobs
WHERE skill ILIKE '%SQL%'
	AND days_since_posting > 21
	AND domain IS NOT NULL
GROUP BY domain
ORDER BY require_sql DESC;
